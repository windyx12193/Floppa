--[[ FLOPPA AUTO JOINER v5.8
     • Тянем JSON с https://server-eta-two-29.vercel.app/api/jobs?limit=...
     • MIN M/S в МИЛЛИОНАХ (1 => $1,000,000/s)
     • JOIN ретраи 10/сек, очистка >3 мин, подсветка новых
     • Хоткей T, конфиг floppa_aj_settings.json, авто-реинжект
     • Подробный DEBUG
]]
...
-- Persisted state (load before UI)
local State = {
    AutoJoin=false, AutoInject=false, IgnoreEnabled=false,
    JoinRetry=50, MinMS=1, IgnoreNames={}
}
do
...
local function mkToggle(p,txt,def)
    ...
    local function apply(v,inst) state.Value=v; local pos=v and UDim2.new(1,-26,0.5,-12) or UDim2.new(0,2,0.5,-12); local col=v and COLORS.on or COLORS.off
...
local function mkStackInput(p,title,ph,def,isNum)
    ...
    local state={}; box.FocusLost:Connect(function() state.Value=isNum and (tonumber(box.Text) or 0) or box.Text end)
...
local statsLbl=mkLabel(right,"",13,"medium",COLORS.textWeak); statsLbl.AnchorPoint=Vector2.new(1,0); statsLbl.Position=UDim2.new(1,-10,0,8); statsLbl.Size=UDim2.new(0,280,0,16); statsLbl.TextXAlignment=Enum.TextXAlignment.Right
...
local function saveSettings() saveJSON(SETTINGS_PATH,{AutoJoin=autoJoin.Value,AutoInject=autoInject.Value,IgnoreEnabled=ignoreToggle.Value,JoinRetry=tonumber(jrBox.Text) or State.JoinRetry,MinMS=tonumber(msBox.Text) or State.MinMS,IgnoreNames=parseIgnore(ignoreBox.Text)}) end
    ...
    for idx,id in ipairs(Order) do
        ...
        if e and e.frame then
            ...
            if age<=FRESH_AGE_SEC then e.frame.BackgroundColor3=Color3.fromRGB(22,28,26); e.frame.BackgroundTransparency=ALPHA.panel
                ...
                e.frame.BackgroundColor3=COLORS.surface; local st=math.clamp((os.clock()-e.lastSeen)/ENTRY_TTL_SEC,0,1)
                e.frame.BackgroundTransparency=ALPHA.panel+0.05*st
            end
            shown+=1
        end
    end
...
-- ==== NETWORK: api/jobs ====
local function buildApiUrl(limit)
    limit = limit or 200
    local t = os.time() .. tostring(math.random(1000,9999))
    -- ключ не обязателен для чтения, но если нужен — добавим
    return string.format("%s/api/jobs?limit=%d&key=%s&_cb=%s", SERVER_BASE, limit, API_KEY, t)
end

local function pullOnce()
    local url = buildApiUrl(200)
    local ok, body = pcall(function() return game:HttpGet(url) end)
    if not ok or type(body)~="string" or #body==0 then
        if DEBUG then warn("[Floppa] GET failed:", url) end
        return
    end
    local okJ, data = pcall(function() return HttpService:JSONDecode(body) end)
    if not okJ or type(data)~="table" or type(data.items)~="table" then
        if DEBUG then warn("[Floppa] JSON invalid, body head:", body:sub(1,120)) end
        return
    end
    if DEBUG then print(string.format("[Floppa] fetched items: %d", #data.items)) end
    ...
    for _,it in ipairs(data.items) do
        ...
        if id~="" and name~="" and moneyStr~="" and players~="" then
            ...
            local item = {jobId=id, name=name, moneyStr=moneyStr, mps=mps, curPlayers=cur, maxPlayers=max, ts=ts}
            if passFilters(item) then
                local curBest = bestById[id]
                if (not curBest) or (item.mps > curBest.mps) then bestById[id]=item end
            end
        elseif DEBUG then
            print("[Floppa][skip] bad item:", id, name, moneyStr, players)
        end
    end

    -- apply
    for id, d in pairs(bestById) do updateItem(id, d) end
...
task.spawn(function()
    while gui and gui.Parent do
        ...
        if not ok and DEBUG then warn("[Floppa] pullOnce crashed") end
        task.wait(PULL_INTERVAL_SEC)
    end
end)
refreshBtn.MouseButton1Click:Connect(function() if DEBUG then print("[Floppa] manual refresh") end; pcall(pullOnce) end)
