...
local function saveJSON(path,t) if not hasFS() then return false end local ok,d=pcall(function()return HttpService:JSONEncode(t) end); if not ok then return false end pcall(writefile,path,d); return true end
local function loadJSON(path) if not hasFS() or not isfile(path) then return nil end local ok,d=pcall(readfile,path); if not ok or type(d)~="string" then return nil end local ok2,t=pcall(function()return HttpService:JSONDecode(d) end); if not ok2 then return nil end return t end

-- ===== singleton cleanup =====
local function guiRoot()
    local okH,h=pcall(function() return gethui and gethui() end); if okH and h then return h end
    local okC,c=pcall(function() return game:GetService("CoreGui") end); if okC then return c end
    return Players.LocalPlayer and Players.LocalPlayer:FindFirstChildOfClass("PlayerGui") or nil
end
do
    local r=guiRoot(); if r then local old=r:FindFirstChild("FloppaAutoJoinerGui"); if old then pcall(function() old:Destroy() end) end end
...
do local cfg=loadJSON(SETTINGS_PATH)
   if cfg then
...
surface=Color3.fromRGB(18,18,22),surface2=Color3.fromRGB(26,26,32),textPrimary=Color3.fromRGB(238,238,245),textWeak=Color3.fromRGB(190,190,200),
on=Color3.fromRGB(64,222,125),off=Color3.fromRGB(120,120,130),joinBtn=Color3.fromRGB(67,232,113),stroke=Color3.fromRGB(70,60,140)}
...
local function mkToggle(p,txt,def)
    ...
    local state={Value=def and true or false}
    local function apply(v,inst) state.Value=v
...
local function updLeftCanvas() left.CanvasSize=UDim2.new(0,0,0,leftList.AbsoluteContentSize.Y+leftPad.PaddingBottom.Offset) end
leftList:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updLeftCanvas)
...
local function persist() saveJSON(SETTINGS_PATH,{AutoJoin=autoJoin.Value,AutoInject=autoInject.Value,IgnoreEnabled=ignoreToggle.Value,JoinRetry=tonumber(jrBox.Text) or State.JoinRetry,MinMS=tonumber(msBox.Text) or State.MinMS,IgnoreNames=parseIgnore(ignoreBox.Text)}) end
...
local function parseMoneyStr(s)s=tostring(s or ""):gsub(",",""):upper(); local n,u=s:match("%$%s*([%d%.]+)%s*([KMBT]?)%s*/%s*[Ss]"); if not n then n,u=s:match("%$%s*([%d%.]+)%s*([KMBT]?)") end; if not n then return 0 end; return math.floor((tonumber(n) or 0)*(mult[u or ""] or 1)+0.5) end
...
local function passFilters(d)
    ...
    if ignoreToggle.Value and #State.IgnoreNames>0 then for _,nm in ipairs(State.IgnoreNames) do if #nm>0 and d.name:lower():find(nm:lower(),1,true) then return false end end end
    return true
end
