-- FLOPPA AUTO JOINER - Safe Mode (минимум зависимостей)
-- • Горячая клавиша: T (eng)
-- • Конфиг: workspace/.floppa_aj/config.txt (минимальный текст)
-- • Без bit32, без шифрования — максимум совместимости
-- • AutoInject через queue_on_teleport
-- • Фиолетовый GUI, лёгкий блюр, тумблеры и поля

local AUTO_INJECT_URL = "https://raw.githubusercontent.com/windyx12193/Floppa/main/aj.lua"
local FIXED_HOTKEY    = Enum.KeyCode.T

local function log(...) print("[FloppaSafe]", ...) end
local function warnf(...) warn("[FloppaSafe][ERR]", ...) end
...
local ok_main, err_main = pcall(function()
    ...
    local function saveConfig(cfg)
        ...
        local ok, e = pcall(writefile, CFG_TXT, body)
        if not ok then warnf("writefile failed:", e) end
        return ok
    end
    ...
    local function loadConfig()
        ...
        for line in (txt.."\n"):gmatch("(.-)\n") do
            ...
            if k and v then
                k = k:gsub("%s+"," "):upper()
                if k=="MIN M/S" then cfg.MinMS=tonumber(v) or cfg.MinMS
                elseif k=="A/J" then cfg.AutoJoin=(v=="true" or v=="True")
                elseif k=="JOIN RETRY" then cfg.JoinRetry=tonumber(v) or cfg.JoinRetry
                ...
                elseif k=="AUTO INJECT" then cfg.AutoInject=(v=="true" or v=="True")
                end
            end
        end
        return cfg
    ...
    mkLabel(header,"FLOPPA AUTO JOINER (SAFE)",20,"bold",COLORS.textPrimary).Size=UDim2.new(0.7,0,1,0)
    ...
    if State.AutoInject then
        local q=pickQueue(); if q then q(makeBootstrap(AUTO_INJECT_URL)) end
    end
...
if not ok_main then
    warn("[FloppaSafe][FATAL]", err_main)
end
