--[[
  FLOPPA AUTO JOINER - Luau-safe v4.3 HOTFIX
  • Singleton: один экземпляр GUI
  • AUTO INJECT асинхронно (без фриза UI), queue_on_teleport
  • Если URL указывает на этот же aj.lua — не запускаем «сейчас», только queue на телепорт
  • Запоминание хоткея в getgenv().__FLOPPA_HOTKEY
]]
...
local function mkToggle(parent, text, default)
    ...
    dot.Size=UDim2.new(0,24,0,24); dot.Position=UDim2.new(0,2,0.5,-12); dot.BackgroundColor3=COLORS.off; dot.Parent=sw; roundify(dot,12)
...
-- === Root GUI (gethui/CoreGui) ===
local parent
local okH, hui = pcall(function() return gethui and gethui() end)
if okH and hui then parent = hui else
    local okC, core = pcall(function() return game:GetService("CoreGui") end)
    parent = okC and core or player:WaitForChild("PlayerGui")
end
-- удалить возможный старый GUI
local old = parent:FindFirstChild("FloppaAutoJoinerGui")
if old then old:Destroy() end

local gui=Instance.new("ScreenGui")
gui.Name="FloppaAutoJoinerGui"; gui.IgnoreGuiInset=true; gui.ResetOnSpawn=false; gui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling; gui.DisplayOrder=1e6
gui.Parent=parent

local main=Instance.new("Frame")
main.Name="Main"; main.Size=UDim2.new(0,980,0,560); main.Position=UDim2.new(0.5,-490,0.5,-280)
...
hotkeyInfo.AnchorPoint=Vector2.new(1,0.5); hotkeyInfo.Position=UDim2.new(1,-60,0.5,0); hotkeyInfo.Size=UDim2.new(0,220,1,0); hotkeyInfo.TextXAlignment=Enum.TextXAlignment.Right

local keyButton=Instance.new("TextButton")
keyButton.Size=UDim2.new(0,36,0,32); keyButton.AnchorPoint=Vector2.new(1,0.5); keyButton.Position=UDim2.new(1,-18,0.5,0)
...
left.Size=UDim2.new(0,300,1,-58); left.Position=UDim2.new(0,0,0,58); left.BackgroundTransparency=1
...
jrBox.FocusLost:Connect(function() State.JoinRetry=tonumber(jrBox.Text) or State.JoinRetry end)
msBox.FocusLost:Connect(function() State.MinMS=tonumber(msBox.Text) or State.MinMS end)
ignoreState.Changed=function(text) State.IgnoreNames=parseIgnore(text) end

-- === Auto Inject (async, без дублей GUI) ===
...
-- === Show/hide + rebind (сохранение хоткея) ===
local opened=true
