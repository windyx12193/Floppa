-- FLOPPA LAST-LINE AJ (light, no-freeze)
...
-- ========== settings (START OFF) ==========
local S = { started=false, minProfitM=1, lastSeenBottomId="" }
...
-- ========== HTTP (cache buster, без лишних заголовков) ==========
...
-- ========== tiny parser (только последняя строка) ==========
...
-- безопасно берём последнюю НЕпустую строку
...
local function parse_last(body)
    local line = last_line(body)
...
-- ========== UI (как на скрине, без тяжёлых эффектов) ==========
local gui=Instance.new("ScreenGui")
gui.Name="FLOPPA_LAST_LINE_AJ"
...
-- ========== teleport (единственные логи — ретраи) ==========
...
-- ========== main loop (минимум работы за тик) ==========
        ...
        if not body or #body==0 then
            ...
            elseif id == S.lastSeenBottomId then
                ...
                if it and it.cur < it.max and it.profitM >= S.minProfitM then
                    ...
                    if ok then
                        setStarted(false) -- на новом сервере автозагрузится
                        joining=false
                        return
                    else
