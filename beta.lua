--[[ FLOPPA LAST-LINE AJ — FEED version
     Источник: https://server-eta-two-29.vercel.app/api/feed?limit=120 (публичный)
     - Парсер строк "Name | **$.../s** | **X/Y** | UUID"
     - Фильтр по M/s, cur < max
     - Ретраи телепорта: до 65 раз, 10/сек
     - UI (Min profit + START/STOP), логи в F9
     - JSON-автосейв (старт всегда OFF), авто-инжект beta.lua
--]]
...
local function log(s)  print(string.format("[%s] %s", ts(), tostring(s))) end
local function warnf(s) warn(string.format("[%s] %s", ts(), tostring(s))) end
...
------------------ PARSER ------------------
-- приводим все прибыли к М/с
local MULT = {K=1/1000, M=1, B=1000, T=1e6}
local function trim(s) return (s:gsub("^%s+",""):gsub("%s+$","")) end
local function clean(s) return (s:gsub("%*%*",""):gsub("\226\128\139","")) end -- убираем ** и zero-width
...
frame.Size = UDim2.fromOffset(300, 150); frame.Position = UDim2.new(0,20,0,100)
...
title.BackgroundTransparency = 1; title.Text = "FLOPPA LAST-LINE AJ (feed)"
...
statusLbl.Size = UDim2.new(1,-20,0,20); statusLbl.Position = UDim2.new(0,10,0,112)
...
local function run_loop()
    ...
    while Settings.started do
        ...
        local candidates = {}
        local count, okUUID, okProfit, okPlayers = 0, 0, 0, 0
        if body and #body>0 then
            for s in string.gmatch(body, "[^\r\n]+") do
                s = trim(s)
                if #s>0 then
                    count += 1
                    if parse_uuid_anywhere(s) then okUUID+=1 end
                    if parse_profit_anywhere(s) then okProfit+=1 end
                    local c,m = parse_players_anywhere(s); if c and m then okPlayers+=1 end

                    local it = parse_line(s)
                    if it and it.cur < it.max and it.profitM >= Settings.minProfitM and not triedJob[it.jobId] then
                        table.insert(candidates, it)
                    end
                end
            end
            log(("feed ok: %d lines; uuid:%d profit:%d players:%d; >=%.2fM/s => %d candidates")
                :format(count, okUUID, okProfit, okPlayers, Settings.minProfitM, #candidates))
        else
        ...
        if #candidates == 0 then
            ...
            for _,cand in ipairs(candidates) do
                if not Settings.started then break end
                triedJob[cand.jobId] = true
                local msg = ("join %s | %.2fM/s | %d/%d | %s"):format(cand.name or "?", cand.profitM or 0, cand.cur or 0, cand.max or 0, cand.jobId)
                ...
                if ok then
                    ...
                    log("server full? retry limit reached ("..tries..") — try next")
                end
                task.wait(0.25)
            end
        end
